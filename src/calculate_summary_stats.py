...
def drop_and_create_summary_table(cursor, turbine_ids):
    try:
...
def get_anomalies_summary_stats(connection):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            """
            if not drop_and_create_summary_table(cursor, turbine_ids):
...
def calculate_summary_stats(connection):
    ...
    try:
        ...
        print(f"calculate_summary_stats failed with ValueError: {e}")
        logging.error(f"calculate_summary_stats failed with ValueError: {e}")
        return False

    except TypeError as e:
        print(f"calculate_summary_stats failed with TypeError: {e}")
        logging.error(f"calculate_summary_stats failed with TypeError: {e}")
        return False
